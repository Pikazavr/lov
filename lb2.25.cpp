#include <stdio.h>
#include <stdlib.h>
#include <time.h>  // Для оценки времени и генерации случайных чисел
...
// Функция для генерации массива с возрастающей и убывающей последовательностью
void generate_increasing_decreasing_array(int* items, int count)
{
    // Заполнение первой половины возрастающими числами
    for (int i = 0; i < count / 2; i++) {
        items[i] = i;  // Возрастающая последовательность
    }
    // Заполнение второй половины убывающими числами
    for (int i = count / 2; i < count; i++) {
        items[i] = count - 1 - (i - count / 2);  // Убывающая последовательность
    }
}
...
int main() {
    ...
    int count = 10;  // Вы можете изменить это значение для большего массива
    ...
    printf("\nTime Shell sort: %.6f sec\n", shell_time);

    // Снова генерируем массив с возрастающей и убывающей последовательностью для быстрой сортировки
    generate_increasing_decreasing_array(items, count);

    // Вывод нового массива
    printf("\nNew array (increasing-decreasing):\n");
    for (int i = 0; i < count; i++) {
        printf("%d ", items[i]);
    }
    printf("\n");
